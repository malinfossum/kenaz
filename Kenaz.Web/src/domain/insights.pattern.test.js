...
test("bestDay picks max mood; null when window empty or no mood", () => {
	...
	const best = bestDay(
		[rec(TODAY, { mood: 5 }), rec(back(1), { mood: 9 }), rec(back(2), { mood: 3 })],
		NOW
	)
...
test("bestDay ignores null-mood days and the 7th day back", () => {
	expect(
		bestDay([rec(TODAY, { mood: 5 }), rec(back(1), { note: "x" }), rec(back(2), { mood: 7 })], NOW)
			.date
	).toBe(back(2))
...
test("bestDay tie-breaks to the most recent date", () => {
	expect(
		bestDay([rec(back(3), { mood: 7 }), rec(back(1), { mood: 7 }), rec(back(2), { mood: 7 })], NOW)
			.date
	).toBe(back(1))
})
test("worstDay picks min mood; tie-breaks to most recent", () => {
	expect(
		worstDay([rec(TODAY, { mood: 5 }), rec(back(1), { mood: 3 }), rec(back(2), { mood: 9 })], NOW)
			.date
	).toBe(back(1))
	expect(
		worstDay([rec(back(3), { mood: 2 }), rec(back(1), { mood: 2 }), rec(back(2), { mood: 2 })], NOW)
			.date
	).toBe(back(1))
})
...
test("pattern buckets at threshold with inclusive long side (7 = long, 6.99 = short)", () => {
	const p = sleepMoodPattern(
		[rec(TODAY, { mood: 7, sleep: 7 }), rec(back(1), { mood: 6, sleep: 6.99 })],
		NOW
	)
	expect(p.longSleepDays).toBe(1)
...
test("pattern confident when both buckets meet 5 exactly, with averages", () => {
	...
	expect(p).toMatchObject({
		longSleepDays: 5,
		shortSleepDays: 5,
		longSleepMoodAverage: 8,
		shortSleepMoodAverage: 5,
		isConfident: true,
	})
})
