...
test("empty store: no week data, null averages, streak 0, no highlights/teaser", () => {
	...
	expect(i).toMatchObject({
		...
		hardestDay: null,
		sleepThreshold: 7,
		sleepPatternConfident: false,
		showSleepTeaser: false,
		teaserDirection: "None",
	})
})
test("week averages over the 7-day window", () => {
	const i = computeInsights(
		[rec(TODAY, { mood: 8, energy: 6, sleep: 7 }), rec(back(2), { mood: 4, energy: 4, sleep: 5 })],
		NOW
	)
...
test("highlights need two DISTINCT mood days", () => {
	expect(computeInsights([rec(TODAY, { mood: 9 }), rec(back(1), { mood: 3 })], NOW)).toMatchObject({
		hasHighlights: true,
		brightestDay: { date: TODAY },
		hardestDay: { date: back(1) },
	})
...
test("brightestDay carries no note field", () => {
	const i = computeInsights(
		[rec(TODAY, { mood: 9, note: "great" }), rec(back(1), { mood: 3 })],
		NOW
	)
...
test("teaser: confident + gap >= 1 → MoreSleepBetter", () => {
	...
	expect(computeInsights(c, NOW)).toMatchObject({
		sleepPatternConfident: true,
